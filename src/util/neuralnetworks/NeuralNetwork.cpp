...
NeuralNetwork::NeuralNetwork(std::vector<double>& weights,
    ...
    , _requiredNumberOfWeights(0)
    , _haveInputsChanged(false)
    , _haveOutputsChanged(false)
...
const std::vector<double>& NeuralNetwork::readOut() const { return _outputs; }
...
unsigned int NeuralNetwork::getNbInputs() const { return _nbInputs; }

unsigned int NeuralNetwork::getNbOutputs() const { return _nbOutputs; }
