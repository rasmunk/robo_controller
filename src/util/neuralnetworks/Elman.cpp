...
#include <include/util/neuralnetworks/Elman.h>
#include <math.h>
#include <sstream>
...
void Elman::step()
{
    ...
    for (unsigned int k = 0; k < _nbNeuronsPerLayer.size() - 1; k++) {
        ...
        if (remainingWeights < prec.size() + nbBiais) {
            std::stringstream ss;
            ss << "remainingWeights = " << remainingWeights << " but at least "
               << prec.size() + nbBiais << " weights are needed...";
...
std::string Elman::getNNTypeName() { return "Elman"; }

Elman* Elman::clone() const { return new Elman(*this); }
